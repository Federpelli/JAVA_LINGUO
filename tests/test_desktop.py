...
class DesktopConfigurationTests(unittest.TestCase):
    def test_product_name_and_versions_are_synchronized(self) -> None:
        ...
        main_window = tauri_config["app"]["windows"][0]
        self.assertFalse(main_window["fullscreen"])
        self.assertTrue(main_window["maximized"])
        self.assertTrue(main_window["decorations"])
        self.assertTrue(main_window["resizable"])
    ...
    def test_signed_tauri_updater_is_configured(self) -> None:
        ...
        self.assertIn("allow-minimize-main-window", loopback_capability["permissions"])
        self.assertIn("allow-load-course-progress", loopback_capability["permissions"])
        self.assertIn("allow-save-course-progress", loopback_capability["permissions"])
        self.assertIn(
        ...
        progress_store = (ROOT / "src-tauri" / "src" / "progress_store.rs").read_text()
        ...
        self.assertIn("course-progress-v1.json", progress_store)
        self.assertIn("java-linguo-progress-v3", progress_store)
        self.assertIn("recover_legacy_directory", progress_store)
        self.assertIn("MAX_PROGRESS_BYTES", progress_store)
        self.assertIn("AppManifest::new().commands", build_script)
        self.assertIn('"minimize_main_window"', build_script)
        self.assertIn('"load_course_progress"', build_script)
        self.assertIn('"save_course_progress"', build_script)
        ...
        self.assertIn("invoke('minimize_main_window')", escape_shortcut)
        self.assertIn("appWindow.minimize()", escape_shortcut)
        self.assertIn("addEventListener('keyup'", escape_shortcut)
        ...
        self.assertIn("behavior: 'auto'", page)
        self.assertIn("block: 'start'", page)
        self.assertIn("lessonHeadingRef.current?.focus({ preventScroll: true })", page)
        self.assertIn("async function fetchWithTimeout(", page)
        self.assertIn("'/api/lab/status'", page)
        self.assertIn("'/api/lab/command'", page)
        self.assertIn("window.setTimeout(persistWithoutWaiting, 120)", page)
        self.assertIn("loadNativeCourseProgress()", page)
        self.assertIn("saveNativeCourseProgress(payload)", page)
